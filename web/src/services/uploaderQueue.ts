...
export class UploaderQueue {
  ...
  addSegment(
    segment: Omit<QueuedSegment, 'retryCount' | 'maxRetries' | 'status'>
  ): void {
  ...
  private async processQueue(): Promise<void> {
    ...
    const uploadPromises = pendingSegments.map(segment =>
      this.uploadSegment(segment)
    )
  ...
  private async uploadSegment(segment: QueuedSegment): Promise<void> {
    ...
    try {
      ...
      if (segment.retryCount < segment.maxRetries) {
        ...
        segment.onError?.(new Error(`Failed after ${segment.maxRetries} retries: ${segment.lastError}`))
      }
    }
  }
